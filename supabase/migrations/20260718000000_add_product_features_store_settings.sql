...
DROP POLICY IF EXISTS "pv auth read" ON public.product_variants;
CREATE POLICY "pv auth read" ON public.product_variants FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "pv member write" ON public.product_variants;
...
DROP POLICY IF EXISTS "pvv public read" ON public.product_variant_values;
CREATE POLICY "pvv public read" ON public.product_variant_values FOR SELECT TO anon USING (true);
DROP POLICY IF EXISTS "pvv auth read" ON public.product_variant_values;
CREATE POLICY "pvv auth read" ON public.product_variant_values FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "pvv member write" ON public.product_variant_values;
