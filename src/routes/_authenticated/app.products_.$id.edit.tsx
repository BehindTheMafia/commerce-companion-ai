...
export const Route = createFileRoute("/_authenticated/app/products_/$id/edit")({
...
function EditProductPage() {
  ...
  const { data: product, isLoading: productLoading } = useQuery({
    ...
    enabled: !!activeBusiness && !!productId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("products")
        .select(
          "id,name,slug,sku,price,sale_price,stock,status,image_url,description,category_id,pricing_modes,specifications,shipping_info,warranty_info,wholesale_info",
        )
        .eq("id", productId)
        .single();
  ...
  const { data: categories = [] } = useQuery({
    queryKey: ["categories-select-edit", activeBusiness?.id],
  ...
  const { data: brands = [] } = useQuery({
    queryKey: ["brands-select-edit", activeBusiness?.id],
    enabled: !!activeBusiness,
    queryFn: async () => {
      const { data } = await supabase
        .from("brands")
  ...
  useEffect(() => {
    ...
    setSku((product.sku as string) ?? "");
    setDescription((product.description as string) ?? "");
    setPrice((product.price as number)?.toString() ?? "");
    setSalePrice((product.sale_price as number)?.toString() ?? "");
    setStock((product.stock as number)?.toString() ?? "0");
    setCategoryId((product.category_id as string) ?? "");
    setImageUrl((product.image_url as string) ?? "");
    setStatus((product.status as string) ?? "draft");
    setPricingModes(parseModes());
    setSpecs(parseSpecs());
    setShippingInfo((product.shipping_info as string) ?? "");
    setWarrantyInfo((product.warranty_info as string) ?? "");
    setWholesaleInfo((product.wholesale_info as string) ?? "");
    setLoaded(true);
  }, [product]);
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-3 pr-8">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <Input
                              value={mode.badge}
                              onChange={(e) => updatePricingMode(mode.id, "badge", e.target.value)}
                              placeholder="Bulk"
                              className="h-9 text-sm"
                            />
                          </div>
                ...
                <div className="lg:sticky lg:top-24 lg:space-y-6">
                  ...
                  <Card className="border-border/40 p-5">
                    ...
                    <div className="space-y-3 text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <span>{status === "active" ? "Publicado" : "Borrador"}</span>
                      </div>
                    </div>
                  </Card>
                  ...
                  <div className="pt-2">
                    <Button
                      ...
                      size="sm"
                      disabled={deleting}
                      className="w-full gap-2 bg-destructive text-destructive-foreground hover:bg-destructive/90 text-xs h-8 disabled:opacity-60"
                      onClick={async () => {
                        if (
